...
export default function CommunityHoursPage() {
  ...
  async function refreshTotal(sid: number) {
    ...
    try {
      const res = await safeGet(endpoints.community.studentHours(sid), () => mockHoursProgress(sid, REQUIRED_COMMUNITY));
      setMode(res.mode);
  ...
  const missing = Math.max(0, REQUIRED_COMMUNITY - total);
  const completed = total >= REQUIRED_COMMUNITY;

  const traffic = trafficFromHours(total, REQUIRED_COMMUNITY);

  return (
    <AppShell>
      <div className="flex items-start justify-between gap-6 flex-col md:flex-row">
        <div>
          <div className="text-2xl font-bold">Community Hours</div>
          <div className="text-sm text-slate-400 mt-1">Register and track community service hours.</div>
        </div>
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          <CardHeader title="Current status" subtitle={`Required: ${REQUIRED_COMMUNITY} hours`} />
        ...
        <Card>
          ...
          <CardBody className="space-y-4">
            {role === "ADMIN" ? (
              ...
              <div className="text-sm text-slate-400">
                If you already completed a program, wait for the administrator to update your hours.
                You can view available programs in <span className="text-slate-200">Community → Programs</span>.
              </div>
            )}
          </CardBody>
        </Card>
